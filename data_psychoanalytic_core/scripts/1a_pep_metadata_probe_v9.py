...
from typing import Any, Dict, Iterable, List, Optional, Tuple, Tuple, Tuple
...
def make_params(job: Dict[str, Any], limit: int, offset: int = 0) -> Dict[str, Any]:
    """
    Build a confirmed PEP Search API query.
...
def payload_shape(payload: Any) -> Dict[str, Any]:
    if isinstance(payload, dict):
        out: Dict[str, Any] = {"payload_type": "dict", "top_level_keys": list(payload.keys())[:30]}
...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    parser.add_argument("--diagnose", action="store_true", help="Print safe diagnostics without exposing credentials.")
    parser.add_argument("--facetquery", help="Override generated facetquery for one-off API syntax or article-id tests.")
    ...
    if not args.dry_run:
        if getattr(args, "append_flat_csv", False):
        ...
        if getattr(args, "write_run_csv", False) and all_rows:
            run_dir = PROJECT_ROOT / "data" / "source_recon" / "probe_runs"
            run_dir.mkdir(parents=True, exist_ok=True)
            journal_part = args.journal[0] if args.journal else "all_journals"
            year_part = str(args.year[0]) if args.year else "all_years"
            stamp = dt.datetime.now().strftime("%Y%m%d_%H%M%S") if "dt" in globals() else datetime.now().strftime("%Y%m%d_%H%M%S")
