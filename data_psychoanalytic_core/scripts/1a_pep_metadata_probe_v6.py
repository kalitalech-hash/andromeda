...
Credentials
-----------
Put credentials or authorization values in data_psychoanalytic_core/.env.pep,
not in scripts/.env.pep. This local file must NOT be committed to Git.
See .env.pep.example.
...
def build_headers() -> Dict[str, str]:
    ...
    # Generic authorization variants.
    ...
    # PEP-Web specific headers observed in browser/API requests.
    # Environment variable names use uppercase and underscores.
    pep_header_map = {
...
def safe_secret_state(env_path: Path, headers: Dict[str, str]) -> Dict[str, Any]:
    """Return non-sensitive diagnostics about env/header loading."""
    return {
...
def payload_shape(payload: Any) -> Dict[str, Any]:
    """Summarize response structure without printing payload content."""
    if isinstance(payload, dict):
        out: Dict[str, Any] = {
            "payload_type": "dict",
            "top_level_keys": list(payload.keys())[:30],
        }
        for key in ["documentList", "responseSet", "documents", "results", "count", "fullCount", "total", "error", "message", "detail"]:
...
def make_params(job: Dict[str, Any], limit: int, offset: int = 0) -> Dict[str, Any]:
    ...
    - PEPCode is returned in records but is not a searchable field.
    - journal filtering works with article-id prefix: art_id:IJP.*
    - year filtering works with: year:2020
    """
    year = job["year"]
    pep_code = job.get("pep_code_candidate")

    if pep_code:
        facetquery = f"art_id:{pep_code}.* AND year:{year}"
    else:
        # Fallback: sourceTitle may or may not be searchable in PEP's Solr schema.
        # Keep it only for reconnaissance and diagnose failures explicitly.
...
def append_flat_csv(path: Path, rows: List[Dict[str, Any]]) -> None:
    """Append rows to an existing CSV, creating it if needed."""
    if not rows:
        return
    path.parent.mkdir(parents=True, exist_ok=True)
...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    parser.add_argument("--append-flat-csv", action="store_true", help="Append records to flat CSV instead of overwriting it.")
    parser.add_argument("--write-run-csv", action="store_true", help="Also write a per-run flat CSV file under data/source_recon/probe_runs/.")
    ...
    parser.add_argument("--diagnose", action="store_true", help="Print safe diagnostics: env/header presence, HTTP status, response shape.")
    parser.add_argument("--facetquery", help="Override generated facetquery for one-off API syntax tests.")
    ...
    if args.diagnose:
        print(json.dumps({"safe_env_header_diagnostics": safe_secret_state(env_path, headers)}, ensure_ascii=False, indent=2))
    session = requests.Session()
    ...
    if not args.dry_run:
        ...
        if args.write_run_csv and all_rows:
            ...
            journal_part = args.journal or "all_journals"
            year_part = str(args.year) if args.year else "all_years"
