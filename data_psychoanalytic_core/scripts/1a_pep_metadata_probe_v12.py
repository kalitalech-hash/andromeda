...
DEFAULT_KEYWORDS_LONG_CSV = PROJECT_ROOT / "data" / "source_recon" / "psychoanalytic_core_keywords_probe_long.csv"
...
def extract_document_info_xml_meta(record: Dict[str, Any]) -> Dict[str, str]:
    """Extract selected bibliographic metadata from PEP documentInfoXML.

    Confirmed fields in IJP.101.0013A:
    <artinfo arttype="ART" j="IJP" doi="..." id="...">
      <artpgrg>13-41</artpgrg>
      ...
    </artinfo>
    """
    xml_text = record.get("documentInfoXML") or record.get("documentMetaXML") or ""
    ...
    try:
        root = ET.fromstring(xml_text)
        out["article_type_xml"] = root.attrib.get("arttype", "") or ""
        out["doi_xml"] = root.attrib.get("doi", "") or ""
        out["document_id_xml"] = root.attrib.get("id", "") or ""

        pgrg = root.find(".//artpgrg")
        if pgrg is not None:
            out["pages_xml"] = normalize_keyword("".join(pgrg.itertext()))
    except Exception:
        # Regex fallbacks for imperfect XML snippets.
        m = re.search(r'<artinfo[^>]*\barttype=["\']([^"\']+)["\']', xml_text, flags=re.IGNORECASE)
        if m:
            out["article_type_xml"] = normalize_keyword(m.group(1))
        m = re.search(r'<artinfo[^>]*\bdoi=["\']([^"\']+)["\']', xml_text, flags=re.IGNORECASE)
        if m:
            out["doi_xml"] = normalize_keyword(m.group(1))
        m = re.search(r'<artinfo[^>]*\bid=["\']([^"\']+)["\']', xml_text, flags=re.IGNORECASE)
        if m:
            out["document_id_xml"] = normalize_keyword(m.group(1))
        m = re.search(r'<artpgrg[^>]*>(.*?)</artpgrg>', xml_text, flags=re.IGNORECASE | re.DOTALL)
        if m:
            out["pages_xml"] = normalize_keyword(m.group(1))

    return out


def get_stat_value(record: Dict[str, Any], key: str) -> Any:
...
def flatten_record(job: Dict[str, Any], record: Dict[str, Any], source_file: str) -> Dict[str, Any]:
    ...
    xml_meta = extract_document_info_xml_meta(record)

    document_id = (
        get_nested(record, ["documentID", "id", "art_id"])
        or xml_meta.get("document_id_xml")
        or ""
    )
    pages = (
        get_nested(record, ["pages", "pageRange"])
        or xml_meta.get("pages_xml")
        or ""
    )
    doi = (
        get_nested(record, ["doi", "DOI"])
        or xml_meta.get("doi_xml")
        or ""
    )
    article_type = (
        get_nested(record, ["articleType", "documentType", "type"])
        or xml_meta.get("article_type_xml")
        or ""
    )
    ...
    return {
        ...
        "art_kwds_count": get_stat_value(record, "art_kwds_count"),
        "source_file": source_file,
    }



def flatten_keyword_rows(article_row: Dict[str, Any]) -> List[Dict[str, Any]]:
    """Create keyword-long rows from one flattened article row.

    This is the preferred analytical format for keyword QA and later semantic
    normalization. One row = one article-keyword assignment.
    """
    joined = article_row.get("keywords_joined") or ""
    if not joined:
        return []

    keywords = [normalize_keyword(k) for k in joined.split(";")]
    keywords = [k for k in keywords if k]

    rows: List[Dict[str, Any]] = []
    for idx, keyword in enumerate(keywords, start=1):
        rows.append({
            "journal_id": article_row.get("journal_id", ""),
            "journal_label": article_row.get("journal_label", ""),
            "pep_code_candidate": article_row.get("pep_code_candidate", ""),
            "probe_year": article_row.get("probe_year", ""),
            "document_id": article_row.get("document_id", ""),
            "pep_code_record": article_row.get("pep_code_record", ""),
            "year_record": article_row.get("year_record", ""),
            "title": article_row.get("title", ""),
            "authors": article_row.get("authors", ""),
            "source_title": article_row.get("source_title", ""),
            "volume": article_row.get("volume", ""),
            "issue": article_row.get("issue", ""),
            "pages": article_row.get("pages", ""),
            "doi": article_row.get("doi", ""),
            "article_type": article_row.get("article_type", ""),
            "keyword_order": idx,
            "keyword_raw": keyword,
            "keyword_source": article_row.get("keyword_source", ""),
            "art_kwds_count": article_row.get("art_kwds_count", ""),
            "source_file": article_row.get("source_file", ""),
        })
    return rows


def write_keywords_long_csv(path: Path, rows: List[Dict[str, Any]]) -> None:
    ...
    fieldnames = [
        ...
        "keyword_order", "keyword_raw", "keyword_source", "art_kwds_count",
        "source_file",
    ]
...
def append_keywords_long_csv(path: Path, rows: List[Dict[str, Any]]) -> None:
    if not rows:
        return
    path.parent.mkdir(parents=True, exist_ok=True)
...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    parser.add_argument("--keywords-long-csv", default=str(DEFAULT_KEYWORDS_LONG_CSV))
    ...
    keywords_long_csv = Path(args.keywords_long_csv)

    session = requests.Session()
    session.headers.update(headers)

    all_rows: List[Dict[str, Any]] = []
    all_keyword_rows: List[Dict[str, Any]] = []
    ...
    for idx, job in enumerate(jobs, start=1):
        ...
        for record in records:
            article_row = flatten_record(job, record, source_file=str(raw_path.relative_to(PROJECT_ROOT)))
            all_rows.append(article_row)
            all_keyword_rows.extend(flatten_keyword_rows(article_row))
    ...
    if not args.dry_run:
        if args.append_flat_csv:
            ...
            append_keywords_long_csv(keywords_long_csv, all_keyword_rows)
            print(f"Appended flattened probe CSV: {flat_csv}")
            print(f"Appended keyword-long probe CSV: {keywords_long_csv}")
        else:
            write_flat_csv(flat_csv, all_rows)
            write_keywords_long_csv(keywords_long_csv, all_keyword_rows)
            print(f"Wrote flattened probe CSV: {flat_csv}")
            print(f"Wrote keyword-long probe CSV: {keywords_long_csv}")
        ...
        if args.write_run_csv and all_rows:
            ...
            run_kw_csv = run_dir / f"pep_probe_keywords_long_{journal_part}_{year_part}_{stamp}.csv"
            write_keywords_long_csv(run_kw_csv, all_keyword_rows)
            print(f"Wrote per-run probe CSV: {run_csv}")
            print(f"Wrote per-run keyword-long CSV: {run_kw_csv}")
        ...
        print(f"Flattened keyword rows: {len(all_keyword_rows)}")
