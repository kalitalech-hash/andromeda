#!/usr/bin/env python3
"""
Andromeda keyword scraper for psychotherapy / clinical psychology journals. v0.3
...
def crossref_query(
    ...
    Notes:
        v0.3 deliberately defaults to not using `select`, because one invalid
        selectable field causes Crossref to reject the whole query. This was the
        likely cause of the first zero-article run.
    """
    ...
    while True:
        try:
            ...
            if use_select and params.get("select"):
                diagnostics["error"] = f"select_query_failed_retrying_without_select: {type(e).__name__}: {e}; body={getattr(e.response, 'text', '')[:500]}"
        ...
        if diagnostics["crossref_total_results"] is None:
            diagnostics["crossref_total_results"] = msg.get("total-results")
